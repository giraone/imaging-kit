/*
 * Original code from ImageJ package. Adapted to latest JRE.
 */
 ...
 * MemoryCacheSeekableStream class. Can also be constructed from a RandomAccessFile, which uses less memory since the
 * memory cache is not required.
 */
public final class RandomAccessStream extends InputStream {
    ...
    public int read() throws IOException {
        ...
        int l1 = readUntil(l);
        if (l1 >= l) {
            byte abyte0[] = (byte[]) data.get((pointer >> BLOCK_SHIFT));
            return abyte0[(pointer++ & BLOCK_MASK)] & 0xff;
        } else
            return -1;
    }
    ...
    public int read(byte[] bytes, int off, int len) throws IOException {
        ...
        if (l <= pointer)
            ...
            byte abyte1[] = (byte[]) data.get((pointer >> BLOCK_SHIFT));
            int k = Math.min(len, BLOCK_SIZE - (pointer & BLOCK_MASK));
            System.arraycopy(abyte1, (pointer & BLOCK_MASK), bytes, off, k);
            pointer += k;
            return k;
        }
    }
